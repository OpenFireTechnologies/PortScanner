...
def main():
    ...
    if(target_host == None):
        banner()
        try:
            print "Usage : %s [options]" % program_name.split("\\")[2]
            print "Check : %s --help for help" % program_name.split("\\")[2]
        except:
            print "Usage : %s [options]" % program_name
            print "Check : %s --help for help" % program_name
        exit(0)
    if threads_no > 800:
        banner()
        print "Threads must be less than 800, default no. of threads= 100"
        exit()
